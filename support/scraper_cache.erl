...
-export([
    empty_scraper_data/2,
    init/1,
    put/6,
    store_ignored_for_attribute/4
]).


empty_scraper_data(Id, Context) ->
    DeleteQuery = io_lib:format("DELETE FROM mod_scraper WHERE scraper_id=~p", [Id]),
    z_db:q(DeleteQuery, Context).


put(Id, Url, ConnectedRscId, Status, Results, Context) ->
    Columns = [
        {scraper_id, Id},
...
store_ignored_for_attribute(Id, Url, RuleId, Context) ->
